...
export function startServer(store) {
	...
	function select(state) {
		...
		console.log("STATE ?", state.filterNot(keyIn('rdb'))); // pick
	}
	...
	function hasStateChanged(newValue) {
	 	...
	 	console.log("compare", previousValue, 'with', newValue);
  	...
  	store.subscribe(() => {
  		...
  		if(hasStateChanged(reducedState) && reducedState.size > 0) {
  			...
  			console.log("store has changed, but nothing interesting really happened");
  		}
   	});
